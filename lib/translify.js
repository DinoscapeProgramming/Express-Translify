(() => {
  ...
  loadConfig().then(async ({ default: defaultLanguage }) => {
    ...
    async function translateTextNode(textNode, { language, reverse }) {
    ...
    async function translateElementNode(elementNode, { language, reverse }) {
      const dictionary = (!reverse) ? ((await translations?.[(language || navigator.language || "en").split("-")[0]]) || {}) : Object.fromEntries(Object.entries((await translations?.[reverse]) || {}).map(([key, value]) => [
        value, key
      ]));
    ...
    window.translify = async (language) => {
      if ((navigator.language || defaultLanguage).split("-")[0] !== defaultLanguage) await scanAndTranslate(document.body, { reverse: (navigator.language || "en").split("-")[0] });
