...
(() => {
  ...
  const initialEmitter = new EventTarget();

  window.translify = async (language) => {
    currentLanguage = language.split("-")[0];

    await new Promise((resolve) => initialEmitter.addEventListener("translationCompleted", resolve));
  ...
  loadConfig().then(async ({ default: defaultLanguage, languages }) => {
    ...
    if (currentLanguage) {
      if (![...[defaultLanguage], ...languages].includes(currentLanguage)) {
        scanAndTranslate(document.body);

        throw new InvalidLocaleError(currentLanguage);
      };

      scanAndTranslate(document.body, { language: currentLanguage });

      initialEmitter.dispatchEvent(new CustomEvent("translationCompleted"));
    } else {
      scanAndTranslate(document.body);
    };

    window.translify = async (language) => {
      if (![...[defaultLanguage], ...languages].includes(language.split("-")[0])) throw new InvalidLocaleError(language.split("-")[0]);

      if ((currentLanguage || userLanguage) !== defaultLanguage) await scanAndTranslate(document.body, { reverse: (currentLanguage || userLanguage) });
