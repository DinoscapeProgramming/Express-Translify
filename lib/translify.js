(() => {
  ...
  const cache = new Map(Object.entries(JSON.parse(localStorage.getItem("locales") || "{}")).map(([key, value = []] = []) => [key, value[1]]).filter((locale) => locale));
  ...
  async function loadLanguage(lang) {
    if (cache.has(lang) && (cache.get(lang)[0] === hash)) return cache.get(lang)[1];

    const dataPromise = fetchJSON(`locales/${lang}.json`).catch(() => ({}));

    cache.set(lang, [hash, dataPromise]);

    setTimeout(async () => {
      localStorage.setItem("locales", JSON.stringify(Object.fromEntries(await Promise.all(Array.from(cache).map(async (locale) => [locale[0], [hash, await locale[1][1]]])))));
    }, 0);
  ...
  const translations = new Proxy({}, {
    get(target, lang) {
      ...
      if (!cache.has(lang) || (cache.get(lang)[0] !== hash)) {
        cache.set(lang, [hash, loadLanguage(lang)]);
